...
public class ProdutoDAOTest {
	...
	public void deveSomarTodosOsPrecosPorTipoLivro() {
		...
		List<Produto> livrosImpressos = ProdutoBuilder
				.newProduto(TipoPreco.IMPRESSO, BigDecimal.TEN).more(3)
				.buildAll();
		...
		BigDecimal valorImpresso = produtoDAO
				.somaPrecosPorTipo(TipoPreco.IMPRESSO);
		BigDecimal valorEbook = produtoDAO.somaPrecosPorTipo(TipoPreco.EBOOK);

		assertEquals(new BigDecimal(40).setScale(2), valorImpresso);
		assertEquals(new BigDecimal(40).setScale(2), valorEbook);
	}
}
